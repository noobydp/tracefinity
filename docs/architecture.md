...
│   │       ├── tool_labeler.py           # optional Ollama naming for traced polygons
...
When `TOOL_LABEL_PROVIDER` is enabled, `tool_labeler.py` runs as a background task after contour extraction for both AI tracing and manual mask upload. It crops each polygon from the corrected source image, masks everything outside the contour to white, asks the configured provider for one short JSON tool name per crop, validates it, and writes each result back to `Polygon.label` as it returns only while the session is still waiting for labels and only for polygons that still have generic fallback labels. Polygon edits, explicit cancellation, or saving tools cancel pending label application so late results do not rename a trace after the user accepted the visible names. Supported providers are `ollama`, `gemini`/`google`, `openrouter`, and `hosted` (Gemini first, OpenRouter fallback). Failures are non-fatal and keep the generic fallback labels.
